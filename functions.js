...
document.addEventListener('keydown',function(event) {
  ...
  if(inGame){
    switch (event.key){
      case 'ArrowLeft':
        if (actual.columna > 0){
          if (numero === matrix[fila][actual.columna - 1]){
            reiniciar_casilla(fila, actual.columna);
            matrix[fila][actual.columna] = 0;
            actual.columna--;
            sumar_casillas(numero);
            acomodarNumeros(actual.columna);
            actualizar_numero_movimientos();
          } else if (matrix[fila][actual.columna - 1] === 0){
            reiniciar_casilla(fila, actual.columna);
            matrix[fila][actual.columna] = 0;
            actual.columna--;
            mover_numero_de_casilla(fila, actual.columna, numero);
...
function comprobar_condicion_partida(matrix, columna, numero){
  return matrix[0][columna] === 0 || matrix[0][columna] === numero;
}
...
async function sumar_casillas(numero){
  actual.numero *= 2;                            //numero ingresado multiplicado por dos
  matrix[actual.fila][actual.columna] = actual.numero;//se coloca el numero duplicado en la casilla (efecto de union)
  mover_numero_de_casilla(actual.fila, actual.columna, actual.numero);   //efecto de union en la parte grafica
}
...
async function unir_numeros_vertical(numero, fila, columna, matrix){
  ...
  while(actual.fila < matrix.length - 1 && actual.numero === matrix[actual.fila + 1][actual.columna]){
...
async function acomodarNumeros(columna){
  for(let i = 0; i < matrix.length - 1; i++){ //hace el for 4 veces para asegurar que se haga el proceso bien (se puede mejorar)
    for(let fila = matrix.length - 1; fila > 1; fila--){ //empieza a recorrer desde la ultima fila hasta la fila 2
      ...
      let numero = matrix[filaArriba][actual.columna];        //se obtiene el numero que este por arriba de la fila actual

      if(numero !== 0){                                     //comprueba que la casilla de arriba no este vacia
        if(matrix[fila][actual.columna] === 0){     //comprueba si la casilla actual esta vacia, para bajar el numero de arriba              
          matrix[fila][actual.columna] = numero;    //le coloca a la casilla actual el numero de arriba
          mover_numero_de_casilla(fila, actual.columna, numero); //lo hace graficamente
          matrix[filaArriba][actual.columna] = 0;   //le pone un cero a la casilla de arriba
          reiniciar_casilla(filaArriba, actual.columna);  //lo hace graficamente
        }
        else if(numero === matrix[fila][actual.columna]){ //comprueba si la casilla actual tiene el mismo numero que la de arriba, para unirlos
          matrix[filaArriba][actual.columna] = 0;         //pone la casilla de arriba vacia
          reiniciar_casilla(filaArriba, actual.columna);        //lo hace graficamente
          actual.fila = fila;                                     
          sumar_casillas(numero);                                    //suma las casillas y las une
        }
      }
    }
  }
